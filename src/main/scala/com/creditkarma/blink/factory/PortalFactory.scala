...
/**
  ...
  * The factory class instance will receive all properties based on prefix of the property name,
  * and it then can decide how to create the portal using the properties.
  * The design is very flexible in the sense that the factory class itself has full control of the entire creation process.
  * It can use its own factory/reflection based strategies to create a wider class of portals.
  * And it can decide how to detect and handle configuration errors, etc.
